  ...
  <div class="print-container">
    ...
    <div class="print-content" id="printContent">
      ...
      <div class="print-header">
        <h1 class="print-title">衡水市第三人民医院退货单</h1>
      </div>
      ...
      <div class="print-info">
        <table class="info-table">
          <tr>
            ...
            <td class="info-value">{{ formData.returnNo || '--' }}</td>
...
<script>
import { getEquipmentReturn } from "@/api/equipment/equipmentReturn";

export default {
  name: "EquipmentReturnPrint",
  data() {
    return {
      returnId: null,
      formData: {},
  ...
  methods: {
    // 加载数据
    loadData() {
      getEquipmentReturn(this.returnId).then(response => {
        if (response.code === 200 && response.data) {
          this.formData = response.data;
          this.detailList = response.data.detailList || [];
        } else {
          this.$modal.msgError('获取退货单数据失败');
          this.handleBack();
        }
      }).catch(error => {
        console.error('加载退货单数据失败:', error);
        this.$modal.msgError('加载退货单数据失败');
        this.handleBack();
      });
    },
    // 格式化日期（只显示日期部分）
