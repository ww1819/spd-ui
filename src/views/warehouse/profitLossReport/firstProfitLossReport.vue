  ...
  <div class="app-container list-page first-inventory-page pl-report-query">
    ...
    <div class="table-container" ref="tablePanel">
      ...
      >
        ...
        <el-table-column label="单位" align="left" header-align="center" prop="unitName" width="80" show-overflow-tooltip resizable class-name="ctk-col-left">
          <template slot-scope="scope">
            <span>{{ scope.row.unitName || '--' }}</span>
          </template>
        </el-table-column>
        <el-table-column label="批次号" align="left" header-align="center" prop="batchNo" width="160" resizable class-name="ctk-col-left batch-col-wrap">
          <template slot-scope="scope">
...
import { exportProfitLossEntryStyledXlsx } from '@/utils/departmentOutSummaryExport'
...
export default {
  ...
  data() {
    return {
      ...
      tableHeight: 400,
      moreSearchOptions: [
        { value: "billNo", label: "盈亏单号" },
        { value: "stocktakingNo", label: "盘点单号" },
  ...
  computed: {
    moreSearchStorageKey() {
      return "spd.warehouse.profitLoss.detail.moreSearchTypes";
    },
    builtInMoreSearchDefaults() {
      return ["billNo", "stocktakingNo", "warehouse", "materialName"];
    },
    pageTotalQty() {
  ...
  methods: {
    ...
    applyMoreSearchToQueryParams(target) {
      ...
      if (!set.has("stocktakingNo")) target.stocktakingNo = null;
    ...
    getList() {
      ...
      listProfitLossEntry(params).then(response => {
    ...
    async handleExport() {
      ...
      this.loading = true
      try {
        const response = await listProfitLossEntry(requestParams)
