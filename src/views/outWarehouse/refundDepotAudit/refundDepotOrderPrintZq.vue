<template>
  <div class="refund-depot-order-print receipt-print" ref="receiptRefundDepotOrderPrintRef">
    <div
    ...
    >
      <div class="doc-header">
        ...
        <div class="doc-title">
          <span v-if="hospitalName">{{ hospitalName }}</span>退库单
        </div>
        <div class="page-meta">
      ...
      <div class="info-block" :style="tableStyle">
        <div class="info-grid">
          <div class="info-cell">
            <span class="info-label info-label--left">仓库</span>
            <span class="info-value">{{ row.warehouseName || '' }}</span>
          </div>
          <div class="info-cell info-cell--bill-no">
            <span class="info-label info-label--right">单号</span>
            <span class="info-value">{{ row.billNo || '' }}</span>
          </div>
          <div class="info-cell">
            <span class="info-label info-label--left">科室</span>
            <span class="info-value">{{ row.departmentName || '' }}</span>
          </div>
          <div class="info-cell">
            <span class="info-label info-label--right">制单日期</span>
            <span class="info-value">{{ formatBillDateTime(row.billCreateTime || row.billDate) }}</span>
          </div>
        </div>
      </div>
      ...
      <table class="detail-table" :style="tableStyle">
        ...
        <tbody>
          <tr v-for="(item, idx) in pageRows" :key="`${pageIndex}-${idx}`">
            ...
            <td class="cell-textual">
              <span class="cell-text">{{ item.supplierName || '' }}</span>
            </td>
          </tr>
      ...
      <div class="sign-block" :style="tableStyle">
        ...
        <div class="sign-item">
          <span class="sign-label">退库人</span>
          <span class="sign-value">{{ row.returnerName || '' }}</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    loadPrintSetting() {
      const billType = this.billType || (this.row && this.row.billType) || 401
    ...
    applyPrintCellAutoFont() {
      ...
      const minPx = 8
      const maxSteps = 48
      const ratios = [0.09, 0.14, 0.11, 0.12, 0.10, 0.06, 0.08, 0.08, 0.10, 0.12]
      ...
      const doPrint = () => {
        this.$nextTick(() => {
          requestAnimationFrame(() => {
            const el = this.$refs.receiptRefundDepotOrderPrintRef || this.$el
            if (!el) return
...
.info-cell--bill-no .info-label--right
  flex-shrink 0
