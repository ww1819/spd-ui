        ...
        <el-table :data="gzOrderEntryList" :row-class-name="rowGzOrderEntryIndex"
          ...
          <el-table-column label="备注" prop="remark" width="200" show-overflow-tooltip resizable>
            ...
            <div v-show="action" class="modal-footer">
              ...
              <el-button type="primary" class="spd-btn spd-btn--primary" :disabled="isAuditedForm" @click="submitForm">保 存</el-button>
...
export default {
  ...
  computed: {
    moreSearchStorageKey() {
      return "spd.gzOrder.goodsAudit.moreSearchTypes";
    },
  ...
  methods: {
    ...
    buildDialogSnapshot() {
      ...
      return JSON.stringify({
        ...
        list: (this.gzOrderEntryList || []).map(item => ({
          id: item.id || null,
    ...
    handleBatchPrint() {
      const selected = this.orderList.filter(item => this.ids.includes(item.id));
      const printable = selected.filter(item => item.orderStatus === '2' || item.orderStatus === 2);
    ...
    handleExport() {
      ...
      this.applyMoreSearchToQueryParams(queryParams);
      queryParams.orderType = 301;
      this.download('gz/order/export', queryParams, `order_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
