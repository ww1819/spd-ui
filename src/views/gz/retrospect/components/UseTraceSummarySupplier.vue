...
<script>
import { listTraceSummarySupplier } from "@/api/gz/traceability";
import { exportTraceSummarySupplierTable } from "../retrospectExport";
...
export default {
  ...
  methods: {
    ...
    async exportTable() {
      this.loading = true;
      try {
        const result = await exportTraceSummarySupplierTable(this, this.queryParams);
    ...
    getList() {
      this.loading = true;
      listTraceSummarySupplier(this.buildSummaryQuery()).then(response => {
        const pageBase = ((this.queryParams.pageNum || 1) - 1) * (this.queryParams.pageSize || 10);
        this.summaryList = (response.rows || []).map((row, idx) => {
          if (row && !row._rowKey) {
            const supplierName = row.supplierName || (row.supplier && row.supplier.name) || '';
            row._rowKey = `gz-retrospect-supplier-${pageBase + idx}-${supplierName}-${(row.material && row.material.code) || ''}-${row.id || idx}`;
          }
          return row;
        });
