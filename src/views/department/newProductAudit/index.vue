<template>
  <div class="app-container list-page new-product-audit-page" :class="{ 'is-modal-open': open }">
    ...
    <el-table ref="applyMainTable" v-loading="loading" :data="applyList" class="table-compact apply-main-table"
      ...
      <el-table-column label="操作" align="center" class-name="apply-action-col small-padding fixed-width" width="180">
        <template slot-scope="scope">
          <!-- 已审核的单据只显示打印按钮 -->
          <el-button
...
export default {
  ...
  methods: {
    ...
    fillListTotalAmtFallback(queryParams) {
      ...
      listNewProductAudit({ ...queryParams, pageNum: 1, pageSize }).then(res => {
    ...
    selectableAuditRow(row) {
      return row.applyStatus == 0 || row.applyStatus === '0';
    },
    /** 查看按钮操作 */
    handleView(row){
      // 尝试多种可能的id字段名
