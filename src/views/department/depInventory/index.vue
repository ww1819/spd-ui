<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="inventoryList" @selection-change="handleSelectionChange" height="54vh" border>
...
<script>
import { listInventory, getInventory, delInventory, addInventory, updateInventory } from "@/api/department/depInventory";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除科室库存编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('department/inventory/export', {
        ...this.queryParams
