...
export default {
  ...
  methods: {
    ...
    fillListTotalAmtFallback(queryParams) {
      ...
      listConsume({ ...queryParams, pageNum: 1, pageSize }).then(res => {
    ...
    handleMainRowDblclick(row, column) {
      if (!row) return;
      if (column && (column.type === 'selection' || column.label === '操作' || column.property === 'consumeBillNo')) {
        return;
      }
      const table = this.$refs.applyMainTable;
      if (!table) return;
