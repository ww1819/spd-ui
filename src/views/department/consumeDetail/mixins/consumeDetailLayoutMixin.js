export default {
  ...
  methods: {
    ...
    getConsumeRowKey(row) {
      return (row && row._rowKey) || '';
    },
    consumeRowClassName({ row }) {
      const key = this.getConsumeRowKey(row);
      if (key && this.selectedRowKeys.indexOf(key) !== -1) {
        return 'consume-row-selected';
      }
      return '';
    },
    handleSelectionChange(selection) {
      this.selectedRowKeys = (selection || []).map(row => this.getConsumeRowKey(row));
    },
    handleConsumeRowDblclick(row) {
      ...
      const selected = !!(key && (
        ...
        storeSelection.some(r => this.getConsumeRowKey(r) === key)
      ));
