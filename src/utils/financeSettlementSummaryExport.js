...
import { applyAmountExcelCell, getPriceExcelNumFmt } from '@/utils/moneyFormat';
...
export async function exportFinanceSettlementSummaryXlsx(options) {
  ...
  const writeSection = (categoryLabel, rows, totalAmt) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      applyAmountExcelCell(c, rows[i].wholesaleAmt);
      c.font = FONT_BODY;
    ...
    applyAmountExcelCell(cTot, totalAmt != null ? totalAmt : 0);
...
export async function exportFinanceDeptConsumablePickupXlsx(options) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    cells.forEach(({ c, v, num, align }) => {
      ...
      if (num) applyAmountExcelCell(cell, v);
      setCellBorder(cell);
    });
    ws.getRow(r).height = 18;
    r++;
  ...
  if (rows.length === 0) {
    ...
    ;[
      ...
      if (typeof v === 'number') applyAmountExcelCell(cell, v);
      setCellBorder(cell);
    });
    ws.getRow(r).height = 18;
    r++;
